...
let possibleMoves = [[1,2],[1,-2],[-1,2],[-1,-2],[2,1],[2,-1],[-2,1],[-2,-1]]
 ...
 let currentPossibleMove = [node.data[0] + moves[0], node.data[1] + moves[1]]

  if(myKnight.move(node.data, currentPossibleMove) != false) { //checking if node (just made, can go there)
    ...
    if((visitedNodes.find(element => element === currentPossibleMove)) === currentPossibleMove) {
      return;
    } else {
